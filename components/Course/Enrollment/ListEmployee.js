...
export default function ListEmployee({ created_by }) {
    const [selectedEmployees, setSelectedEmployees] = useState([]); // ✅ Isi: array of no_ktp
    ...
    // ✅ Select all pakai no_ktp
    const handleSelectAll = (checked) => {
        if (checked) {
            setSelectedEmployees(filteredEmployees.map(emp => emp.no_ktp));
        } else {
            setSelectedEmployees([]);
        }
    };

    // ✅ Select individual pakai no_ktp
    ...
    const handleSaveGroupings = async () => {
        ...
        // ✅ selectedEmployees sudah berisi no_ktp
        ...
        const result = await confirmAction({
            title: 'Assign Groupings?',
            text: `This will assign ${selectedGroupings.length} grouping(s) to ${selectedEmployees.length} employee(s)`,
    ...
    return (
        <div className="space-y-6">
                            ...
                            {filteredEmployees.slice(0, pageSize).map((employee) => (
                                ...
                                >
                                    <td className="px-6 py-4">
                                        {/* ✅ Checkbox pakai no_ktp, tapi tampilan tetap sama */}
                                        <input
                                            type="checkbox"
                                    ...
                                    <td className="px-6 py-4">
                                        {/* ✅ Tetap tampilkan employee_id (NIK) */}
            ...
            {/* Grouping Modal - Sama seperti sebelumnya */}
            {showGroupingModal && (
