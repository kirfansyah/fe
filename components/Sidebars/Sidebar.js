...
export default function Sidebar() {
  const sidebarItems = [
    ...
    { icon: Users, label: 'Employee Course', link :'/course/management' },
          ...
          {sidebarItems.map((item, index) => (
             <Link
              ...
              className={`w-full flex items-center px-12 py-4 rounded-lg text-left transition-colors duration-200 ${
                item.active ? 'bg-slate-300 font-medium' : 'hover:bg-gray-100 hover:text-gray-900'
              }`}
            >
              <item.icon className={`w-5 h-5 mr-3 ${item.active ? 'text-blue' : 'text-gray-600'}`} />
              <span className={`font-medium ${item.active ? 'text-blue' : 'text-gray-600'}`}>{item.label}</span>
            </Link>
          ))}
        </nav>
      </div>
