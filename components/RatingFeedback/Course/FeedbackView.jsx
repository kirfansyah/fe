...
export default function FeedbackView({ feedbacks, loading, error, onRefresh }) {
  ...
  const uniqueCourses = [...new Set(feedbacks?.map((f) => f.course_title) || [])];
  ...
  const applyFilters = () => {
    ...
    if (filters.searchText) {
      ...
      filtered = filtered.filter((f) => f.feedback?.toLowerCase().includes(searchLower) || f.course_title?.toLowerCase().includes(searchLower) || f.company_name?.toLowerCase().includes(searchLower));
    }
    ...
    <div className="space-y-6">
      ...
      {filteredFeedbacks && filteredFeedbacks.length > 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          {!loading && (
            <button onClick={onRefresh} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
              Load Feedback Data
            </button>
          )}
        </div>
