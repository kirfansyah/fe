...
export default function RatingView({ ratings, loading, error, onRefresh }) {
  ...
  const uniqueCourses = [...new Set(ratings?.map((r) => r.course_title) || [])];
  ...
  const applyFilters = () => {
    ...
    if (filters.course) {
      filtered = filtered.filter((r) => r.course_title === filters.course);
    }

    if (filters.searchText) {
      const searchLower = filters.searchText.toLowerCase();
      filtered = filtered.filter((r) => r.course_title?.toLowerCase().includes(searchLower) || r.company_name?.toLowerCase().includes(searchLower));
    }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 bg-gray-50 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                {showFilters ? (
                  <>
                    <X className="w-4 h-4" />
              ...
              <div className="flex bg-gray-100 rounded-lg p-1">
                <button onClick={() => setViewMode("card")} className={`p-2 rounded-md transition-colors ${viewMode === "card" ? "bg-white text-blue-600 shadow-sm" : "text-gray-600 hover:text-gray-900"}`} title="Card View">
                  <LayoutGrid className="w-4 h-4" />
                </button>
                <button onClick={() => setViewMode("list")} className={`p-2 rounded-md transition-colors ${viewMode === "list" ? "bg-white text-blue-600 shadow-sm" : "text-gray-600 hover:text-gray-900"}`} title="List View">
        ...
        {showFilters && (
          <div className="p-4 space-y-4">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search course or company..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={filters.course} onChange={(e) => handleFilterChange("course", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button onClick={applyFilters} className="px-6 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors">
                Apply Filters
              </button>
            </div>
          </div>
